...
/* Test for the motors() command (specifically timing) on Sparki and with python */
...
float getSerialFloat() {
  ...
  char buf[size];
  int result = getSerialBytes(buf, size);
  printDebug(result, DEBUG_DEBUG, 1);
  delay(2000);
  return atof( buf );
}
...
//  printDebug("Using motors to move forward for 1.5 seconds", DEBUG_INFO, 1);
//  delay(2000);
  motors(100, 100, 1.5);
...
//  printDebug("Using motors to turn right for 1.5 seconds", DEBUG_INFO, 1);
//  delay(2000);
  motors(100, -100, 1.5);
